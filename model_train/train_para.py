...
from utils import glue_compute_metrics
from utils import glue_output_modes,glue_tasks_num_labels
from model import JointQAModel
from glue_dataset import GlueDataset


# from transformers.modeling_roberta import RobertaModel, RobertaClassificationHead, RobertaConfig
...
class ModelArguments:
    ...
    data_lim: Optional[int] = field(
        default=None, metadata={"help": "Max number of samples"}
    )
    ...
    use_triple_loss: Optional[bool] = field(
        default=False, metadata={"help": "use triple loss in the training"}
    )
...
def main():
    ...
    eval_dataset = (
        GlueDataset(data_args, tokenizer=tokenizer, mode="dev", cache_dir=model_args.cache_dir,data_lim=model_args.data_lim)
